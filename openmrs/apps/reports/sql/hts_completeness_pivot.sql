SELECT DISTINCT
	   ...
	   round((SUM(IF(HTS_Consultations_vs_Registrations_Agg.status = 'Consulted', 1, 0))/SUM(1))*100) AS '% with HTS Consultations',
	   round((SUM(IF(HTS_Consultations_vs_Registrations_Agg.status = 'Not Consulted', 1, 0))/SUM(1))*100) AS '% without HTS Consultations'	   
FROM
(SELECT DISTINCT
					reg.location_name
					, reg.id
			...
			FROM
					(
						(SELECT DISTINCT
							...
							FROM visit v
								...
								JOIN visit_type vt on v.visit_type_id=vt.visit_type_id AND vt.visit_type_id = 15
					...
					FROM visit v
							...
							JOIN obs o on o.encounter_id=en.encounter_id 
							JOIN location l on v.location_id = l.location_id and l.retired=0
                            Where o.concept_id = 2386
                            AND MONTH(o.obs_datetime) = MONTH(CAST('#endDate#' AS DATE)) 
                            AND YEAR(o.obs_datetime) = YEAR(CAST('#endDate#' AS DATE))) hts_consultation
