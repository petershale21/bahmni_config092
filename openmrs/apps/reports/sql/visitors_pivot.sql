SELECT Total_Aggregated_Visitors.Age_Group
		, Total_Aggregated_Visitors.Seen_Male 
		, Total_Aggregated_Visitors.Seen_Female
		, Total_Aggregated_Visitors.Total

FROM

(
	(SELECT VISITORS_DETAILS.Age_Group AS 'Age_Group'
			, IF(VISITORS_DETAILS.Id IS NULL, 0, SUM(IF(VISITORS_DETAILS.Status = 'Seen' AND VISITORS_DETAILS.Gender = 'M', 1, 0))) AS Seen_Male
			, IF(VISITORS_DETAILS.Id IS NULL, 0, SUM(IF(VISITORS_DETAILS.Status = 'Seen' AND VISITORS_DETAILS.Gender = 'F', 1, 0))) AS Seen_Female
			, IF(VISITORS_DETAILS.Id IS NULL, 0, SUM(1)) as 'Total'
	...
	(
SELECT Id
     , patientIdentifier as Patient_Identifier
...
(select o.person_id, value_numeric as Days_Dispensed
 ...
 ON visitors.Id = dispensed_days.person_id
	...
	GROUP BY VISITORS_DETAILS.Age_group)
...
FROM
		...
		(SELECT  Total_VISITOR.Id
					, Total_VISITOR.Age
					, Total_VISITOR.Gender
					, Total_VISITOR.Status
