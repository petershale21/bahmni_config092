select age_group,Died_Females,Died_Males,Transferred_Females,Transferred_Males,Stopped_Females,Stopped_Males,LTFU_Under3mnths_Females,LTFU_Under3mnths_Males,LTFU_Above3mnths_Females,LTFU_Above3mnths_Males
FROM(
select age_group
			, IF(Id IS NULL, 0, SUM(IF(Outcome = 'Died' AND gender = 'F', 1, 0))) AS Died_Females
			, IF(Id IS NULL, 0, SUM(IF(Outcome = 'Died' AND gender = 'M', 1, 0))) AS Died_Males
			, IF(Id IS NULL, 0, SUM(IF(Outcome = 'Transferred' AND gender = 'F', 1, 0))) AS Transferred_Females
			, IF(Id IS NULL, 0, SUM(IF(Outcome = 'Transferred' AND gender = 'M', 1, 0))) AS Transferred_Males
			, IF(Id IS NULL, 0, SUM(IF(Outcome = 'Stopped' AND gender = 'F', 1, 0))) AS Stopped_Females
			, IF(Id IS NULL, 0, SUM(IF(Outcome = 'Stopped' AND gender = 'M', 1, 0))) AS Stopped_Males
			, IF(Id IS NULL, 0, SUM(IF(Outcome = 'LTFU_Under3mnths' AND gender = 'F', 1, 0))) AS LTFU_Under3mnths_Females
			, IF(Id IS NULL, 0, SUM(IF(Outcome = 'LTFU_Under3mnths' AND gender = 'M', 1, 0))) AS LTFU_Under3mnths_Males
			, IF(Id IS NULL, 0, SUM(IF(Outcome = 'LTFU_Above3mnths' AND gender = 'F', 1, 0))) AS LTFU_Above3mnths_Females
			, IF(Id IS NULL, 0, SUM(IF(Outcome = 'LTFU_Above3mnths' AND gender = 'M', 1, 0))) AS LTFU_Above3mnths_Males
FROM(
	select Patient_Identifier as Id,Outcome,gender,age_group
	
	FROM(
		(select Patient_Identifier,Patient_Name,gender,age_group,'Died' as Outcome
from(
		(SELECT patientIdentifier AS "Patient_Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Initiated' AS 'Program_Status', sort_order
			FROM
...
select Patient_Identifier,Patient_Name,gender,age_group,'Stopped' as Outcome
from(
		(SELECT patientIdentifier AS "Patient_Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Initiated' AS 'Program_Status', sort_order
			FROM
                (select distinct patient.patient_id AS Id,
	...
	WHERE Newly_Initiated_ART_Clients.Id in (																		 
					...
					from obs 
					inner join(
								select person_id,max(obs_datetime) lastday 
								from obs o
								where obs_datetime <= CAST('#endDate#' AS DATE)
								and concept_id = 3701
								and person_id in(
					...
					on a.person_id = obs.person_id
					where lastday = obs_datetime
					and concept_id = 3701
			)
ORDER BY Newly_Initiated_ART_Clients.Age)
...
select Patient_Identifier,Patient_Name,gender,age_group,'Transferred' as Outcome
from(
		(SELECT patientIdentifier AS "Patient_Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Initiated' AS 'Program_Status', sort_order
			FROM
                (select distinct patient.patient_id AS Id,
	...
	WHERE Newly_Initiated_ART_Clients.Id in (																		 
					...
					from obs o
					where obs_datetime <= CAST('#endDate#' AS DATE)
					and (concept_id = 4155 and value_coded = 2146)
					group by person_id
					) a
					on a.person_id = obs.person_id
					where lastday = obs_datetime
...
UNION
(
select Patient_Identifier,Patient_Name,gender,age_group,'LTFU_Under3mnths' as Outcome
from(
		(SELECT patientIdentifier AS "Patient_Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Initiated' AS 'Program_Status', sort_order
			FROM
                (select distinct patient.patient_id AS Id,
...
(SELECT patientIdentifier AS "Patient_Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Seen' AS 'Program_Status', sort_order
	FROM (
...
UNION
	...
	(SELECT patientIdentifier AS "Patient_Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Seen_Prev_Months' AS 'Program_Status', sort_order
	FROM (
			(select distinct patient.patient_id AS Id,
                                   patient_identifier.identifier AS patientIdentifier,
                                   concat(person_name.given_name, ' ', person_name.family_name) AS patientName,
...
UNION
(
select Patient_Identifier,Patient_Name,gender,age_group,'LTFU_Above3mnths' as Outcome
from(
		(SELECT patientIdentifier AS "Patient_Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Initiated' AS 'Program_Status', sort_order
			FROM
                (select distinct patient.patient_id AS Id,
