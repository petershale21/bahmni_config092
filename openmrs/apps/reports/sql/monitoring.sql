...
(
	...
	FROM
                (select distinct patient.patient_id AS Id,
									   ...
									   floor(datediff(CAST('2022-03-30' AS DATE), person.birthdate)/365) AS Age,
                ...
                from obs o
						...
						AND MONTH(o.value_datetime) = MONTH(CAST('2022-03-30' AS DATE)) 
						AND YEAR(o.value_datetime) = YEAR(CAST('2022-03-30' AS DATE))
						 )
						 ...
						 AND o.person_id not in (
							...
							AND MONTH(os.obs_datetime) = MONTH(CAST('2022-03-30' AS DATE)) 
							AND YEAR(os.obs_datetime) = YEAR(CAST('2022-03-30' AS DATE))
						 )	
						 ...
						 INNER JOIN reporting_age_group AS observed_age_group ON
						  CAST('2022-03-30' AS DATE) BETWEEN (DATE_ADD(DATE_ADD(person.birthdate, INTERVAL observed_age_group.min_years YEAR), INTERVAL observed_age_group.min_days DAY))
							...
							where o.person_id in (
									-- FOLLOW UPS
										...
										from obs oss
													...
													and CAST(oss.obs_datetime AS DATE) <= CAST('2022-03-30' AS DATE)
													and CAST(oss.obs_datetime AS DATE) >= DATE_ADD(CAST('2022-03-30' AS DATE), INTERVAL -13 MONTH)
													group by oss.person_id) firstquery
										inner join (
													select os.person_id,datediff(CAST(max(os.value_datetime) AS DATE), CAST('2022-03-30' AS DATE)) as last_ap
													from obs os
													where concept_id = 3752
													and CAST(os.obs_datetime AS DATE) <= CAST('2022-03-30' AS DATE)
									...
									(
										...
										group by os.person_id
										having latest_transferout <= CAST('2022-03-30' AS DATE)
									) as TOUTS
							)					
		)
					...
					(
						...
						and o.person_id in (
								-- Death
											...
											from person p
											where dead = 1
											and death_date <= CAST('2022-03-30' AS DATE)		
						)
					)
...
FROM
                ...
                from obs o
						...
						select active_clients.person_id
								...
								(select a.person_id, SUBSTRING(MAX(CONCAT(b.obs_datetime, b.value_datetime)), 20) AS latest_follow_up
									...
									and a.obs_datetime <= cast('2022-03-30'as date)
									group by a.person_id
								) as active_clients
								where active_clients.latest_follow_up < cast('2022-03-30' as date)
								and DATEDIFF(CAST('2022-03-30' AS DATE),latest_follow_up) <= 28
		...
		and active_clients.person_id not in (
									select person_id 
									from person 
									where death_date <= cast('2022-03-30' as date)
									and dead = 1
						 )
						 )
						 -- end
                ...
                from obs o
						...
						select active_clients.person_id
								...
								(select B.person_id, B.obs_group_id, B.value_datetime AS latest_follow_up
									...
									and obs_datetime <= cast('2022-03-30' as date)
...
inner join 
		...
		FROM(
					...
					AND o.voided = 0
					and o.obs_datetime <= cast('2022-03-30' as date)
...
left outer JOIN
	...
	(SELECT person_id, 
	 ...
	 -- where latest_follow_up >= cast('2022-03-30' as DATE)
...
-- date blood drawn
	...
	inner join 
		(
		 ...
		 and oss.obs_datetime < cast('2022-03-30' as date)
