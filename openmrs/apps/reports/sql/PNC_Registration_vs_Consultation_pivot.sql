...
SELECT DISTINCT
	   pnc_consultations_vs_Registrations_Agg.location_name AS Location,
	   SUM(1) AS 'Total Visits Registered',	   
	   SUM(IF(pnc_consultations_vs_Registrations_Agg.status = 'Consulted', 1, 0)) AS Consulted,
	   round((SUM(IF(pnc_consultations_vs_Registrations_Agg.status = 'Consulted', 1, 0))/SUM(1))*100) AS '% with PNC Consultations',
	   round((SUM(IF(pnc_consultations_vs_Registrations_Agg.status = 'Not Consulted', 1, 0))/SUM(1))*100) AS '% without PNC Consultations'	   
FROM
(SELECT DISTINCT
					reg.location_name
					, reg.id
			...
			FROM
					...
					ON reg.id = pnc_consultation.id)) AS pnc_consultations_vs_Registrations_Agg
