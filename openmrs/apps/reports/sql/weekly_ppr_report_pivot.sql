...
UNION ALL
 ...
 select distinct patient.patient_id AS Id,
					...
					from obs o
						...
						AND CAST(o.value_datetime AS DATE) >= CAST('2022-01-01' AS DATE)
...
-- NLIC Clients
						...
						-- NOT Transfered In from another Site
						 AND o.person_id not in (
								select os.person_id 
								from obs os
								where (os.concept_id = 2253 AND CAST(o.obs_datetime AS DATE) >= CAST('#startDate#' AS DATE)
													AND CAST(o.obs_datetime AS DATE) <= CAST('#endDate#' AS DATE))
								AND os.voided = 0					
						 )						 
