SELECT  distinct Patient_Identifier,
				 ...
				 Date_Diagnosed
FROM
((SELECT Id,patientIdentifier AS "Patient_Identifier",ART_Number, patientName AS "Patient_Name",DOB, Age, Gender, 'Initiated' AS 'Program_Status'
...
(SELECT Id,patientIdentifier AS "Patient_Identifier",ART_Number, patientName AS "Patient_Name",DOB, Age, Gender, 'Seen' AS 'Program_Status'
...
(SELECT Id,patientIdentifier AS "Patient_Identifier",ART_Number, patientName AS "Patient_Name", DOB,Age, Gender, 'Seen_Previous' AS 'Program_Status'
FROM (
(select distinct patient.patient_id AS Id,
									   patient_identifier.identifier AS patientIdentifier,
									   p.identifier as ART_Number,
...
(SELECT Id,patientIdentifier AS "Patient_Identifier",ART_Number, patientName AS "Patient_Name", DOB, Age, Gender,'MissedWithin28Days' AS 'Program_Status'
FROM
                 (select distinct patient.patient_id AS Id,
									   patient_identifier.identifier AS patientIdentifier,
									   p.identifier as ART_Number,
...
-- ART START	
	left outer join
	(
	select person_id,CAST(value_datetime AS DATE) as ART_Initiation_Date
	from obs where concept_id = 2249 and voided = 0
	)intake_date
	on active_clients.Id = intake_date.person_id
-- Diagnosis Date	
	left outer join
	(
	...
	on active_clients.Id = diagnosis_date.person_id