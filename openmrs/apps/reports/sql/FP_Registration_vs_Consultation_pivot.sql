...
SELECT DISTINCT
	   fp_consultations_vs_Registrations_Agg.location_name AS Location,
	   SUM(1) AS 'Total Visits Registered',	   
	   SUM(IF(fp_consultations_vs_Registrations_Agg.status = 'Consulted', 1, 0)) AS Consulted,
	   round((SUM(IF(fp_consultations_vs_Registrations_Agg.status = 'Consulted', 1, 0))/SUM(1))*100) AS '% with FP Consultations',
	   round((SUM(IF(fp_consultations_vs_Registrations_Agg.status = 'Not Consulted', 1, 0))/SUM(1))*100) AS '% without FP Consultations'	   
		FROM
		(SELECT DISTINCT
							reg.location_name
							, reg.id
							...
							, reg.age
							, reg.identifier
							, IF(fp_consultation.id is not null, 'Consulted', 'Not Consulted') AS status
					FROM
							(
									(SELECT DISTINCT
										p.person_id as id,
									...
									FROM visit v
										...
										JOIN visit_type vt on v.visit_type_id=vt.visit_type_id AND vt.visit_type_id = 22
							...
							LEFT JOIN 
							-- Family Planning CONSULTED CLIENTS
							(SELECT DISTINCT
									p.person_id as id,
									concat(pn.given_name,' ', ifnull(pn.family_name,'')) as name,
									p.gender AS gender,
