SELECT DISTINCT
					...
					, reg.age as "Age"
					, IF(u5_consultation.id is not null, 'Consulted', 'Not Consulted') AS Status
			FROM
					(
						-- Registered Under5 Clients
						(SELECT DISTINCT
								p.person_id as id,
							...
							FROM visit v
								...
								JOIN visit_type vt on v.visit_type_id=vt.visit_type_id AND vt.visit_type_id = 20
