...
IF(ahd_clients.Id IS NULL, 0, SUM(IF(TB_Screening_Status = 'TB_Screened', 1, 0))) AS AHD_Screened_for_TB,
IF(ahd_clients.Id IS NULL, 0, SUM(IF(TPT_Started_Status = 'STARTED TPT', 1, 0))) AS AHD_Started_on_TPT,
IF(ahd_clients.Id IS NULL, 0, SUM(IF(TPT_Completion_Status = 'COMPLETED TPT', 1, 0))) AS AHD_Completed_TPT,
IF(ahd_clients.Id IS NULL, 0, SUM(IF(TB_LAM = 'Positive', 1, 0))) AS AHD_with_Positive_TB_LAM,
IF(ahd_clients.Id IS NULL, 0, SUM(IF(POS_LAM_STARTED_TREATMENT = 'Started Treatment', 1, 0))) AS AHD_with_Positive_TB_LAM_Started_TB_Treatment
-- IF(TB_Screening_Status OR TPT_Started_Status IS NULL, 0, SUM(1)) AS Total,
-- ahd_clients.sort_order
FROM
(SELECT Id,patientIdentifier , patientName, Age, Gender, age_group, sort_order, TB_Screening_Status, TPT_Started_Status, TPT_Completion_Status, TB_LAM, POS_LAM_STARTED_TREATMENT
FROM
