SELECT ART_Status
			, IF(Id IS NULL, 0, SUM(IF(Age_Group = '<15' AND Gender = 'M', 1, 0))) AS Under15_Males
			, IF(Id IS NULL, 0, SUM(IF(Age_Group = '<15' AND Gender = 'F', 1, 0))) AS Under15_Females
			, IF(Id IS NULL, 0, SUM(IF(Age_Group = '>15' AND Gender = 'M', 1, 0))) AS Above15_Males
			, IF(Id IS NULL, 0, SUM(IF(Age_Group = '>15' AND Gender = 'F', 1, 0))) AS Above15_Females
FROM(

SELECT Id,Patient_Name,Program_Status,ART_Status,Gender,Age_Group
from(
-- starting tx,new on ART
(SELECT Id,Patient_Name,Program_Status,ART_Status,Gender,'<15' as 'Age_Group'
FROM
(
(SELECT Id,patientIdentifier AS "Patient Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Initiated' AS 'Program_Status','New_on_ART' as 'ART_Status', sort_order
FROM
                ...
                from obs o
						...
						and person_id in(
										select o.person_id -- ,given_name,family_name
										from obs o
										-- inner join person_name pn on o.person_id = pn.person_id
										where concept_id = 4153
										and o.voided = 0
										and o.person_id in(select person_id from obs where concept_id = 2237 and value_datetime BETWEEN CAST('#startDate#' AS DATE) AND CAST('#endDate#' AS DATE))
...
(
SELECT distinct Id,Patient_Name,Program_Status,ART_Status,Gender,'<15' as 'Age_Group'
FROM ((SELECT Id,patientIdentifier AS "Patient Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Seen' AS 'Program_Status','Already_on_ART' as 'ART_Status', sort_order
FROM (
...
(SELECT Id,patientIdentifier AS "Patient Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Seen_Prev_Months' AS 'Program_Status','Already_on_ART' as 'ART_Status', sort_order
FROM (
(select distinct patient.patient_id AS Id,
                                   patient_identifier.identifier AS patientIdentifier,
                                   concat(person_name.given_name, ' ', person_name.family_name) AS patientName,
