(SELECT distinct patientIdentifier AS "Patient Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Initiated' AS 'Program_Status', sort_order
FROM
                ...
                from obs o
						...
						AND o.person_id in(
									...
									select person_id
									FROM obs
									WHERE (concept_id = 3887 and value_coded in(3816,3817,3818))
									and voided = 0
									...
									select person_id
									FROM obs
									WHERE (concept_id = 3805 and value_coded = 1738)
									and voided = 0
									and obs_datetime BETWEEN CAST('#startDate#' AS DATE) AND CAST('#endDate#' AS DATE)					
									)
									OR person_id in(
...
(SELECT distinct patientIdentifier AS "Patient Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Seen' AS 'Program_Status', sort_order
FROM (
...
(SELECT distinct patientIdentifier AS "Patient Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'MissedWithin28Days' AS 'Program_Status', sort_order
FROM
                (select distinct patient.patient_id AS Id,
									   patient_identifier.identifier AS patientIdentifier,
									   concat(person_name.given_name, ' ', person_name.family_name) AS patientName,
...
(SELECT distinct patientIdentifier AS "Patient Identifier", patientName AS "Patient_Name", Age, Gender, age_group, 'Seen_Prev_Months' AS 'Program_Status', sort_order
FROM (
(select distinct patient.patient_id AS Id,
                                   patient_identifier.identifier AS patientIdentifier,
                                   concat(person_name.given_name, ' ', person_name.family_name) AS patientName,
